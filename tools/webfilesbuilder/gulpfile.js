...
function espIndexJsMinify (cb) {
    return pump([
        gulp.src('../../src/websrc/js/index.js'),
        uglify(),
        gulp.dest('../../src/websrc/gzipped/js/'),
    ] );
}

function espIndexJsGz() {
    return gulp.src("../../src/websrc/gzipped/js/index.js")
        .pipe(gzip({
            append: true
        }))
...
function espIndexJsGzh(cb) {
    var source = "../../src/websrc/gzipped/js/" + "index.js.gz";
    var destination = "../../src/webh/" + "index.js.gz.h";
    ...
    wstream.write('#define index_js_gz_len ' + data.length + '\n');
    wstream.write('const uint8_t index_js_gz[] PROGMEM = {')
...
async function runner() {
    const scriptTasks = gulp.series(espIndexJsMinify, espIndexJsGz, espIndexJsGzh, espMainJsMinify, espMainJsGz, espMainJsGzh, scriptsConcat, scripts);
